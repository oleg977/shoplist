...
# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = True # <--- Исправлено:

# Если DEBUG = False, Django требует указать ALLOWED_HOSTS.
# Для локальной разработки можно указать:
ALLOWED_HOSTS = ["127.0.0.1", "localhost"]  # <--- Добавить

#ALLOWED_HOSTS = ['shoplist.onrender.com']
...
# Указываем, куда перенаправлять после успешного входа (по желанию)
LOGIN_REDIRECT_URL = '/'
...
TEMPLATES = [
    {
        ...
        'DIRS': [BASE_DIR / 'shoplist/templates'],
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
